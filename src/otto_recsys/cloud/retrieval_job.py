"""Managed entry point for one bounded retrieval development experiment."""
...
from otto_recsys.research.retrieval_study import run_study


def run(launch: dict[str, Any], inputs: Path, output: Path) -> dict[str, Any]:
    output.mkdir(parents=True, exist_ok=True)
    logger = configure_logging("retrieval_study", log_dir=output / "logs")
    ...
    try:
        ...
        status["stage"] = "retrieval_ablation"
        ...
        logger.exception("retrieval_study_failed")
        raise
    finally:
        ...
        for handler in logger.handlers:
            handler.flush()
        storage.publish(output / "logs/retrieval_study.jsonl")
    return status
    ...
    print(
        json.dumps(
            run(
                ...
                Path("artifacts/retrieval_study"),
            ),
            indent=2,
        )
    )

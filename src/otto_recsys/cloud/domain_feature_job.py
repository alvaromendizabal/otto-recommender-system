"""Managed entry point for a bounded domain feature ablation."""
...
from otto_recsys.research.domain_feature_study import run_study
...
def run(launch: dict[str, Any], inputs: Path, output: Path) -> dict[str, Any]:
    ...
    logger = configure_logging("domain_feature_study", log_dir=output / "logs")
    ...
    try:
        ...
        status["stage"] = "domain_feature_ablation"
        ...
        logger.exception("domain_feature_study_failed")
        raise
    finally:
        ...
        for handler in logger.handlers:
            handler.flush()
        storage.publish(output / "logs/domain_feature_study.jsonl")
    return status
    ...
    print(
        json.dumps(
            run(
                ...
                Path("artifacts/domain_feature_study"),
            ),
            indent=2,
        )
    )

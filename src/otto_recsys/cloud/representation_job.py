"""Managed entry point for one bounded learned-feature development experiment."""
...
from otto_recsys.research.representation_study import run_study


def run(launch: dict[str, Any], inputs: Path, output: Path) -> dict[str, Any]:
    output.mkdir(parents=True, exist_ok=True)
    logger = configure_logging("representation_study", log_dir=output / "logs")
    ...
    try:
        ...
        status["stage"] = "learned_feature_ablation"
        ...
        logger.exception("representation_study_failed")
        raise
    finally:
        ...
        for handler in logger.handlers:
            handler.flush()
        storage.publish(output / "logs/representation_study.jsonl")
    return status

...
from otto_recsys.research.retrievers import HISTORY_HOURS, commit_file, verified_file
...
def build_retrievers(
    ...
    *,
    ...
    with workspace_lock(output):
        ...
        direction = config.get("direction", "symmetric")
        if direction not in {"symmetric", "forward"}:
            raise ValueError("retrieval direction must be symmetric or forward")
        distance = "b.event_index::INTEGER-a.event_index::INTEGER"
        if direction == "symmetric":
            distance = f"abs({distance})"
            ...
            with Heartbeat(
                ...
                if publish:
                    for artifact in (
                        contract_path,
                        statistics,
                        statistics.with_suffix(".json"),
                        thin,
                        thin.with_suffix(".json"),
                    ):
                        publish(artifact)
                parts = output / "parts"
                    ...
                    connection.execute(f"""COPY (
                        WITH pairs AS (
                          ...
                          WHERE a.aid % {partitions} = {bucket} AND a.aid<>b.aid
                            AND {distance} BETWEEN 1 AND {window}

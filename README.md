...
The ANN execution history records two stopped attempts: argument conversion, then an incorrect sorted-ID requirement. The latest attempt saved all 96 reference-count parts before stopping. Search now uses the validated inverse map and preserves the trained embedding row order. The launcher checks actual catalogue inputs before paid compute and supports verified reference-count reuse across code revisions. [Execution evidence](reports/metrics/two_tower_fold0_ann_launch.json) and the [production catalogue audit](reports/metrics/two_tower_fold0_catalogue.json) distinguish these operational checks from the still-pending full-scale ANN measurements.

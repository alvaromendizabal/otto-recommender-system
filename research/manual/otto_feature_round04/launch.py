...
def collect():
    ...
    index_receipts=[]
    for rp in sorted((out/'index/chunks').glob('*.json')):
        r=json.loads(rp.read_text())
        index_receipts.append({k:r[k] for k in ('start','end','raw_chunk_sha256','sessions_read','study_sessions_excluded','indexed_item_hour_action_rows')})
    if index_receipts:
        (out/'INDEX_PROGRESS.json').write_text(json.dumps({'completed_chunks':len(index_receipts),
            'raw_bytes_processed':index_receipts[-1]['end'],'chunks':index_receipts},indent=2)+'\n')
    files=[]
    ...
    for rel in ('protocol.json','feature_catalog.json','core.py','experiment.py','intent_features.py',
                'report.py','launch.py','notebook_view.py','MANIFEST.sha256',
                'evidence/ROUND02_REFERENCE.json','evidence/ROUND02_CONTROL_STATISTICS.npz',
                'evidence/ROUND02_AUDIT.json','demand_features.py','demand_index.py','analysis_utils.py',
                'evidence/ROUND03_AUDIT.json','evidence/ROUND03_RESULT.json'):
        candidate=ROOT/rel
    ...
    manifest=[];target=ROOT/'otto_round04_return.zip';tmp=target.with_suffix('.assembling')
...
def run_stage(phase):
    ...
    out=ROOT/'outputs';out.mkdir(exist_ok=True)
    if phase=='tests':cmd=[str(python),'-W','error','-m','unittest','discover','-s',str(ROOT/'tests'),'-v']
    ...
    try:
        ...
        if proc.returncode==75:
            raise RuntimeError('PAUSED_CHECKPOINTED: valid completed units saved. Stop here and return the ZIP; no automatic retry.')

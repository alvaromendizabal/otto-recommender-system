...
LABELS={'control_shared':'134-feature saved control',
        'demand_rolling':'+ 75 rolling-demand features',
        'demand_static_ablation':'+ same 75 formulas, frozen snapshot'}
PAIRS=(('demand_rolling','control_shared'),('demand_static_ablation','control_shared'),
       ('demand_rolling','demand_static_ablation'))

def validate_result(out, prior_stats=None):
    r=json.loads((out/'result.json').read_text())
    if r['status']!='ROUND04_SCREEN_COMPLETED' or set(r['arms'])!=set(ARMS):
        raise ValueError('Complete Round04 result required')
...
def create_report(out=OUT, prior_stats=None):
    ...
    f=go.Figure(go.Scatter(x=vals,y=['Rolling demand − control','Frozen demand − control','Rolling − frozen demand'],mode='markers',
    ...
    f=go.Figure()
    f.add_trace(go.Bar(name='Nonzero demand-feature values',x=[x['name'] for x in first],y=[x['support_fraction'] for x in first]))
    f.add_trace(go.Bar(name='Positive demand-feature values',x=[x['name'] for x in first],y=[x['recent_positive_fraction'] for x in first]))
    f.update_layout(title='Training-only demand feature support',barmode='group',yaxis_title='Fraction of candidate rows',height=690,xaxis_tickangle=-70);figures.append(f)
    ...
    title='OTTO Round 04 · Cross-session as-of demand'
    primary=cmp['demand_rolling_minus_control_shared']
    ...
    (out/'round04_report.html').write_text(html)
    save_json(out/'report_receipt.json',{'status':'ROUND04_REPORT_READY','result_sha256':sha(out/'result.json'),
        'plot_hashes':plot_hashes,'charts':7,'new_model_fits':0,'metrics_replayed':True})
    (out/'MILESTONE_REVIEW.md').write_text('# OTTO Round 04\n\n'+
        f"Control {r['arms']['control_shared']['weighted_recall_at_20']:.6f}; primary gain {primary['gain']:+.6f}.\n\n"+
        'Decision: '+primary['decision']+'. Rolling updates are compared against identical fixed-cutoff formulas; all study sessions are excluded from the event source. '+
    ...
    print('RESULT: ROUND04_REPORT_READY',flush=True)
    return r

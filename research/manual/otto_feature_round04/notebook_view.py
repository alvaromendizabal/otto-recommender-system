...
def start():
    ...
    if (receipt['status']!='ROUND04_REPORT_READY' or hashlib.sha256(raw).hexdigest()!=receipt['result_sha256']):
        raise ValueError('Run the report phase successfully first')
    if result['status']!='ROUND04_SCREEN_COMPLETED':raise ValueError('Research comparison not complete')
    ...
    display(Markdown('**Decision:** '+result['comparisons']['demand_rolling_minus_control_shared']['decision']+'\n\n'
                    '**Rolling-demand primary gain versus matched control:** '+f"{result['comparisons']['demand_rolling_minus_control_shared']['gain']:+.6f}"+
...
def show_chart(number):
    ...
    display({'application/vnd.plotly.v1+json':spec,'text/plain':f'Round 04 chart {number} (interactive Plotly)'},raw=True)
...
def finish():
    ...
    persist('ROUND04_NOTEBOOK_REVIEW_COMPLETE')
    from launch import collect
    print('ROUND04_NOTEBOOK_REVIEW_COMPLETE',flush=True)
    return collect()

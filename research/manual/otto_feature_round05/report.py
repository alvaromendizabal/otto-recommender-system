...
LABELS={'control_shared':'134-feature saved control',
        'action_pairs':'+ 27 action-pair transition features',
        'collapsed_action_ablation':'+ 9 action-collapsed transition features'}
PAIRS=(('action_pairs','control_shared'),('collapsed_action_ablation','control_shared'),
       ('action_pairs','collapsed_action_ablation'))

def validate_result(out, prior_stats=None):
    r=json.loads((out/'result.json').read_text())
    if r['status']!='ROUND05_SCREEN_COMPLETED' or set(r['arms'])!=set(ARMS):
        raise ValueError('Complete Round05 result required')
...
def create_report(out=OUT, prior_stats=None):
    ...
    f=go.Figure(go.Scatter(x=vals,y=['Action pairs − control','Collapsed actions − control','Typed − collapsed actions'],mode='markers',
    ...
    f=go.Figure()
    f.add_trace(go.Bar(name='Nonzero transition-feature values',x=[x['name'] for x in first],y=[x['support_fraction'] for x in first]))
    f.add_trace(go.Bar(name='Positive transition-feature values',x=[x['name'] for x in first],y=[x['recent_positive_fraction'] for x in first]))
    f.update_layout(title='Training-only transition feature support',barmode='group',yaxis_title='Fraction of candidate rows',height=690,xaxis_tickangle=-70);figures.append(f)
    ...
    title='OTTO Round 05 · Historical action-pair transitions'
    primary=cmp['action_pairs_minus_control_shared']
    ...
    (out/'round05_report.html').write_text(html)
    save_json(out/'report_receipt.json',{'status':'ROUND05_REPORT_READY','result_sha256':sha(out/'result.json'),
        'plot_hashes':plot_hashes,'charts':7,'new_model_fits':0,'metrics_replayed':True})
    (out/'MILESTONE_REVIEW.md').write_text('# OTTO Round 05\n\n'+
        f"Control {r['arms']['control_shared']['weighted_recall_at_20']:.6f}; primary gain {primary['gain']:+.6f}.\n\n"+
        'Decision: '+primary['decision']+'. Typed adjacent-event support is compared with an action-collapsed ablation; all study sessions are excluded from the historical source. '+
        'Twelve challenger models and six existing control replays; no original-graph/control rebuild, GitHub/cloud writes or submission. '+
        'Feature engineering remains open. Full source and experiment receipts are in this package.\n')
    print('RESULT: ROUND05_REPORT_READY',flush=True)
    return r

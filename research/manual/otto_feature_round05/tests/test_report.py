...
from transition_features import NAMES
...
def make_fixture(root):
    ...
    for a in ('action_pairs','collapsed_action_ablation'):v[a]=v['control_shared'].copy()
    save_arrays(root/'evaluation_statistics.npz',**v)
    arms=['control_shared','action_pairs','collapsed_action_ablation'];rows=[]
    for f in (0,1):
        ix=v['folds']==f
        for a in arms:rows.append({'arm':a,'fold':f,'features':{'control_shared':134,'action_pairs':161,'collapsed_action_ablation':143}[a],**pooled(v[a][ix],v['denominator'][ix])})
    comp={}
    for l,r in [('action_pairs','control_shared'),('collapsed_action_ablation','control_shared'),('action_pairs','collapsed_action_ablation')]:
        comp[l+'_minus_'+r]=compare(v[l],v[r],v['denominator'],v['folds'])
    result={'status':'ROUND05_SCREEN_COMPLETED','fixture_only':True,
...
class Reports(unittest.TestCase):
    ...
    def test_corrupt_score_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            p=Path(d);r=make_fixture(p);r['arms']['action_pairs']['weighted_recall_at_20']+=.01
    ...
    def test_changed_decision_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            p=Path(d);r=make_fixture(p);r['comparisons']['action_pairs_minus_control_shared']['decision']='WINNER'
            (p/'result.json').write_text(json.dumps(r))
            with self.assertRaises(ValueError):validate_result(p)

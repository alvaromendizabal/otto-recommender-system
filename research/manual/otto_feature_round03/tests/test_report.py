...
from recent_mass_features import NAMES
...
def make_fixture(root):
    ...
    for a in ('mass_missing_aware','mass_zero_ablation'):v[a]=v['control_shared'].copy()
    save_arrays(root/'evaluation_statistics.npz',**v)
    arms=['control_shared','mass_missing_aware','mass_zero_ablation'];rows=[]
    for f in (0,1):
        ix=v['folds']==f
        for a in arms:rows.append({'arm':a,'fold':f,'features':134 if a==arms[0] else 170,**pooled(v[a][ix],v['denominator'][ix])})
    comp={}
    for l,r in [('mass_missing_aware','control_shared'),('mass_zero_ablation','control_shared'),('mass_missing_aware','mass_zero_ablation')]:
        comp[l+'_minus_'+r]=compare(v[l],v[r],v['denominator'],v['folds'])
    result={'status':'ROUND03_SCREEN_COMPLETED','fixture_only':True,
...
class Reports(unittest.TestCase):
    ...
    def test_corrupt_score_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            p=Path(d);r=make_fixture(p);r['arms']['mass_missing_aware']['weighted_recall_at_20']+=.01
    ...
    def test_changed_decision_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            p=Path(d);r=make_fixture(p);r['comparisons']['mass_missing_aware_minus_control_shared']['decision']='WINNER'
            (p/'result.json').write_text(json.dumps(r))
            with self.assertRaises(ValueError):validate_result(p)

...
LABELS={'control_shared':'134-feature saved control',
        'rarity_neighbors':'+ 24 rarity-weighted neighbor features',
        'equal_weight_ablation':'+ 24 equal-weight neighbor features'}
PAIRS=(('rarity_neighbors','control_shared'),('equal_weight_ablation','control_shared'),
       ('rarity_neighbors','equal_weight_ablation'))

def validate_result(out, prior_stats=None, expected_sessions=2048):
    r=json.loads((out/'result.json').read_text())
    if r['status']!='ROUND08_SCREEN_COMPLETED' or set(r['arms'])!=set(ARMS):
        raise ValueError('Complete Round08 result required')
...
def create_report(out=OUT, prior_stats=None, expected_sessions=2048):
    ...
    title='OTTO Round 08 · Rarity-weighted historical-session evidence'
    primary=cmp['rarity_neighbors_minus_control_shared']
    ...
    (out/'round08_report.html').write_text(html)
    save_json(out/'report_receipt.json',{'status':'ROUND08_REPORT_READY','result_sha256':sha(out/'result.json'),
        'plot_hashes':plot_hashes,'charts':9,'new_model_fits':0,'metrics_replayed':True})
    (out/'MILESTONE_REVIEW.md').write_text('# OTTO Round 08\n\n'+
    ...
    print('RESULT: ROUND08_REPORT_READY',flush=True)
    return r

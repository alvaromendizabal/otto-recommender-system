# Round 08 — run this milestone manually
...
Start with Round08. It creates the shared historical cache once; Round09 reuses it.
...
Open `01_build_features.ipynb`. Run each numbered code cell with **Shift+Enter**, inspect its output, then proceed. Do not use Run All in this notebook. The first cell prints `KERNEL_READY`; the next runs the tests. The subsequent cells prepare the cohort, extend postings, extend histories, then generate features. The final cell requires `ROUND08_FEATURES_READY`. Save with Ctrl+S.

Open `02_run_comparison.ipynb`. Run one code cell at a time. The gate requires the completed features; the screen replays six original controls before fitting up to twelve challengers. Report generation recalculates statistics and writes nine Plotly payloads. The last cell prints the exact recorded comparison and requires `ROUND08_REPORT_READY`. Save with Ctrl+S.
...
|---|---|---:|
| `tests` | `ROUND08_TESTS_PASSED` | 90 seconds |
| `prepare` | `SHARED_COHORT_READY` | 200 seconds |
| `postings` | `SHARED_POSTINGS_READY` | 200 seconds |
| `histories` | `SHARED_HISTORY_READY` | 200 seconds |
| `features` | `ROUND08_FEATURES_READY` | 260 seconds |
| `screen` | `ROUND08_SCREEN_COMPLETED` | 260 seconds |
| `report` | `ROUND08_REPORT_READY` | 90 seconds |
...
```bash
/opt/conda/bin/python -u "$HOME/otto_feature_round08/launch.py" tests
```

Continue only after `ROUND08_TESTS_PASSED`.

### 2. prepare

```bash
/opt/conda/bin/python -u "$HOME/otto_feature_round08/launch.py" prepare
```

Continue only after `SHARED_COHORT_READY`.

### 3. postings

```bash
/opt/conda/bin/python -u "$HOME/otto_feature_round08/launch.py" postings
```

Continue only after `SHARED_POSTINGS_READY`.

### 4. histories

```bash
/opt/conda/bin/python -u "$HOME/otto_feature_round08/launch.py" histories
```

Continue only after `SHARED_HISTORY_READY`.

### 5. features

```bash
/opt/conda/bin/python -u "$HOME/otto_feature_round08/launch.py" features
```

Continue only after `ROUND08_FEATURES_READY`.

### 6. screen

```bash
/opt/conda/bin/python -u "$HOME/otto_feature_round08/launch.py" screen
```

Continue only after `ROUND08_SCREEN_COMPLETED`.

### 7. report

```bash
/opt/conda/bin/python -u "$HOME/otto_feature_round08/launch.py" report
```

Continue only after `ROUND08_REPORT_READY`.

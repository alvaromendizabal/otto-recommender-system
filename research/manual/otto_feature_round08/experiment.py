...
ARMS=('control_shared', 'rarity_neighbors', 'equal_weight_ablation')
...
def screen(ctx):
    ...
    if (OUT/'result.json').exists():
        ...
        return {'status':'ROUND08_SCREEN_COMPLETED','new_model_fits':0,'models_reused':12}
    ...
    for fi,fold,tr,va,d,groups,yy,xt,mt,st,xv,mv,sv in prepared:
        ...
        for arm in ARMS[1:]:
            xtrain=np.concatenate([xt,mt if arm=='rarity_neighbors' else st],axis=1)
            validation=np.concatenate([xv,mv if arm=='rarity_neighbors' else sv],axis=1)
            allnames=names+list(NAMES if arm=='rarity_neighbors' else GLOBAL_NAMES);hits=np.zeros((len(va),3),dtype=np.int64)
            for j,o in enumerate(OBJECTIVES):
                before_unit()
    ...
    for left,right in [('rarity_neighbors','control_shared'),('equal_weight_ablation','control_shared'),
                       ('rarity_neighbors','equal_weight_ablation')]:
    ...
    result={'status':'ROUND08_SCREEN_COMPLETED','source_commit':COMMIT,'sessions':len(ids),'validation_sessions':len(d),
            ...
            'primary_comparison':'rarity_neighbors_minus_control_shared','secondary_comparisons_are_not_advancement_gates':True,'model_inventory':inv,

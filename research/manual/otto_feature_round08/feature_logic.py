"""Two predeclared historical-session feature hypotheses; no targets, fits or I/O.
...
same unsigned local window. All supports count distinct historical sessions.
"""
...
def names(round_no:int, ablation:bool=False)->tuple[str,...]:
    if round_no not in (8,9):raise ValueError('Round must be 8 or 9')
...
def build(round_no,anchors,candidates,source_ids,history,frequencies):

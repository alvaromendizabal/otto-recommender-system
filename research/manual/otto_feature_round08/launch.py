...
CAPS={'tests':90,'prepare':200,'postings':200,'histories':200,'features':260,'screen':260,'report':90}
...
def collect():
    ...
    for p in sorted(out.rglob('*')):
        ...
        if rel.parts[0] in ('features','models','index','shared') and p.suffix!='.json':continue
    ...
    manifest=[];target=ROOT/'otto_round08_return.zip';tmp=target.with_suffix('.assembling')
...
def _run_stage(phase):
    ...
    if phase not in ('report','bundle'):
        ...
        other=Path.home()/'otto_feature_round09/outputs/runs'
    ...
    if phase in ('prepare','postings','histories','features'):
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: python launch.py tests|prepare|postings|histories|features|screen|report|bundle')

...
LABELS={'control_shared':'134-feature saved control',
        'forward_latent':'+ 24 primary latent-affinity features',
        'unsigned_latent':'+ 24 matched ablation features'}
PAIRS=(('forward_latent','control_shared'),('unsigned_latent','control_shared'),
       ('forward_latent','unsigned_latent'))

def validate_result(out, prior_stats=None, expected_sessions=2048):
    r=json.loads((out/'result.json').read_text())
    if r['status']!='ROUND12_SCREEN_COMPLETED' or set(r['arms'])!=set(ARMS):
        raise ValueError('Complete Round12 result required')
...
def create_report(out=OUT, prior_stats=None, expected_sessions=2048):
    ...
    title='OTTO Round 12 · Directed action-context latent affinities'
    primary=cmp['forward_latent_minus_control_shared']
    ...
    (out/'round12_report.html').write_text(html)
    save_json(out/'report_receipt.json',{'status':'ROUND12_REPORT_READY','result_sha256':sha(out/'result.json'),
        'plot_hashes':plot_hashes,'charts':9,'new_model_fits':0,'metrics_replayed':True})
    (out/'MILESTONE_REVIEW.md').write_text('# OTTO Round 12\n\n'+
    ...
    print('RESULT: ROUND12_REPORT_READY',flush=True)
    return r

...
ARMS=('control_shared', 'forward_latent', 'unsigned_latent')
...
def screen(ctx):
    ...
    if (OUT/'result.json').exists():
        ...
        return {'status':'ROUND12_SCREEN_COMPLETED','new_model_fits':0,'models_reused':12}
    ...
    for fi,fold,tr,va,d,groups,yy,xt,mt,st,xv,mv,sv in prepared:
        ...
        for arm in ARMS[1:]:
            xtrain=np.concatenate([xt,mt if arm=='forward_latent' else st],axis=1)
            validation=np.concatenate([xv,mv if arm=='forward_latent' else sv],axis=1)
            allnames=names+list(NAMES if arm=='forward_latent' else GLOBAL_NAMES);hits=np.zeros((len(va),3),dtype=np.int64)
            for j,o in enumerate(OBJECTIVES):
                before_unit()
    ...
    for left,right in [('forward_latent','control_shared'),('unsigned_latent','control_shared'),
                       ('forward_latent','unsigned_latent')]:
    ...
    result={'status':'ROUND12_SCREEN_COMPLETED','source_commit':COMMIT,'sessions':len(ids),'validation_sessions':len(d),
            ...
            'primary_comparison':'forward_latent_minus_control_shared','secondary_comparisons_are_not_advancement_gates':True,'model_inventory':inv,

...
class Launcher(unittest.TestCase):
    ...
    def test_caps_are_bounded(self):self.assertLessEqual(max(launch.CAPS.values()),300);self.assertEqual(launch.CAPS['screen'],260)
    ...
    def test_bundle_excludes_large_binaries(self):
        ...
        self.put('outputs/shared/history.npz','LARGE_HISTORY');self.put('outputs/representations/primary.npz','PRIVATE_FACTORS');self.put('outputs/result.json','{}');self.put('review/01.json','{}')
        with zipfile.ZipFile(launch.collect()) as z:
            ...
            self.assertFalse(any('/models/' in n or '/features/' in n or '/index/' in n or '/shared/' in n or '/representations/' in n for n in z.namelist()))

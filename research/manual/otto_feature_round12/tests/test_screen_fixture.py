...
class SyntheticScreen(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with mock.patch.object(e,'OUT',cls.out),mock.patch.dict('sys.modules',{'otto_recsys.research.dataset':module}),mock.patch.object(e,'control_prediction',side_effect=counted):
            ...
            events=lm.validate_events(np.array(histrows,np.int64))
            vocabulary,_=lm.select_vocabulary(events)
            values={}
            if e.ROUND==11:
                matrices=lm.incidence(events,vocabulary)
                for arm,use_idf in [('primary',True),('ablation',False)]:
                    qv,tv,_=lm.session_embedding(matrices,use_idf);values[arm]=(qv,tv)
            else:
                ff=[];bb=[]
                for offset in range(1,6):
                    f,b=lm.pair_rows(events,vocabulary,offset);ff.append(f);bb.append(b)
                forward=np.concatenate(ff);unsigned=np.concatenate(ff+bb)
                for arm,rows in [('primary',forward),('ablation',unsigned)]:
                    qs=[];ts=[]
                    for action in range(3):
                        qv,tv,_=lm.transition_embedding(lm.count_pairs(rows,len(vocabulary),action));qs.append(qv);ts.append(tv)
                    values[arm]=(np.stack(qs),np.stack(ts))
            cls.patches=[mock.patch.object(e,'feature_identity',side_effect=cls.identity),
                         mock.patch.object(e,'require',return_value=cls.fixture_data),
                         mock.patch.object(e.latent_models,'load_embeddings',return_value=(vocabulary,values))]
    ...
    def test_latent_features_are_nonzero(self):
        with np.load(self.out/'features/part-000.npz',allow_pickle=False) as z:
            self.assertTrue(z['primary'].any());self.assertTrue(z['ablation'].any())
            self.assertGreater(float(np.abs(z['primary']-z['ablation']).sum()),0.)

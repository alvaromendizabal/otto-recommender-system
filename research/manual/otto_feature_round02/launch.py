...
CAPS={'tests':60,'features':320,'screen':260,'report':60}
...
def collect():
    ...
    for p in sorted(out.rglob('*')):
        ...
        if rel.parts[0] in ('features','models'):continue
    ...
    for rel in ('protocol.json','feature_catalog.json','core.py','replication.py','intent_features.py',
                'report.py','launch.py','notebook_view.py','MANIFEST.sha256',
                'evidence/ROUND01_REFERENCE.json'):
        candidate=ROOT/rel
    ...
    manifest=[];target=ROOT/'otto_round02_return.zip';tmp=target.with_suffix('.assembling')
...
def run_stage(phase):
    ...
    else:cmd=[str(python),'-u',str(ROOT/'replication.py'),phase]

...
def start():
    ...
    if (receipt['status']!='ROUND02_REPORT_READY' or hashlib.sha256(raw).hexdigest()!=receipt['result_sha256']):
        raise ValueError('Run the report phase successfully first')
    if result['status']!='ROUND02_SCREEN_COMPLETED':raise ValueError('Research comparison not complete')
    ...
    display(Markdown('**Decision:** '+result['comparison']['decision']+'\n\n'
                    '**Timing minus matched control:** '+f"{result['comparison']['gain']:+.6f}"+
                    '\n\nFitting-only temporal replication. Not a Kaggle score; no feature promotion.'))
    return result
...
def show_chart(number):
    ...
    spec=json.loads((ROOT/'outputs/plots'/f'{number:02d}.json').read_text())
    display({'application/vnd.plotly.v1+json':spec,'text/plain':f'Round 02 chart {number} (interactive Plotly)'},raw=True)
...
def finish():
    ...
    persist('ROUND02_NOTEBOOK_REVIEW_COMPLETE')
    from launch import collect
    print('ROUND02_NOTEBOOK_REVIEW_COMPLETE',flush=True)
    return collect()

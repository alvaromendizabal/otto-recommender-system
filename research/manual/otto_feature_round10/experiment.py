...
from shared_reuse import context as prepare_context,prior_chunks,require,evidence
from source_contract import HISTORY_END,FIT_END,COMMIT
from feature_logic import names,build
from funnel_features import SummaryCache
...
ARMS=('control_shared', 'ordered_funnel', 'unordered_funnel_ablation')
...
def feature_identity(ctx):
 ...
 return {'round':ROUND,'source_commit':COMMIT,'protocol_sha256':sha(ROOT/'protocol.json'),
   'code_sha256':{n:sha(ROOT/n) for n in ('experiment.py','shared_reuse.py','feature_logic.py','funnel_features.py','core.py')},
 ...
 for old,ids,candidates,base in prior_chunks(ctx):
  ...
  if receipt.exists():
   ...
   for j,sid in enumerate(ids):
    ...
    x,y,diag=build(ROUND,anchors,candidates[j],source_ids,h,freq,cache=cache)
    if s+j<8:
     xx,yy,_=build(ROUND,anchors,candidates[j,::-1],list(reversed(source_ids)),h,freq,cache=cache)
...
def screen(ctx):
    ...
    if (OUT/'result.json').exists():
        ...
        return {'status':'ROUND10_SCREEN_COMPLETED','new_model_fits':0,'models_reused':12}
    ...
    for fi,fold,tr,va,d,groups,yy,xt,mt,st,xv,mv,sv in prepared:
        ...
        for arm in ARMS[1:]:
            xtrain=np.concatenate([xt,mt if arm=='ordered_funnel' else st],axis=1)
            validation=np.concatenate([xv,mv if arm=='ordered_funnel' else sv],axis=1)
            allnames=names+list(NAMES if arm=='ordered_funnel' else GLOBAL_NAMES);hits=np.zeros((len(va),3),dtype=np.int64)
            for j,o in enumerate(OBJECTIVES):
                before_unit()
    ...
    for left,right in [('ordered_funnel','control_shared'),('unordered_funnel_ablation','control_shared'),
                       ('ordered_funnel','unordered_funnel_ablation')]:
    ...
    result={'status':'ROUND10_SCREEN_COMPLETED','source_commit':COMMIT,'sessions':len(ids),'validation_sessions':len(d),
            ...
            'primary_comparison':'ordered_funnel_minus_control_shared','secondary_comparisons_are_not_advancement_gates':True,'model_inventory':inv,
            ...
            'limitations':['Reused Round02 fitting cohort; adaptive exploration, not independent validation.',
              'Pending Round09 and predeclared Round10 independently compare with the immutable 134-feature control.',
              'All original study sessions excluded from certified retained historical tails; source not full raw sessions.',
              '64 recent matching sessions per anchor; retained-tail source and selected-neighbor funnel ratios are not population conversion probabilities.',

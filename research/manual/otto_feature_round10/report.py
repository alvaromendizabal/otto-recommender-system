...
LABELS={'control_shared':'134-feature saved control',
        'ordered_funnel':'+ 24 ordered same-product progression features',
        'unordered_funnel_ablation':'+ 24 unordered same-product co-action features'}
PAIRS=(('ordered_funnel','control_shared'),('unordered_funnel_ablation','control_shared'),
       ('ordered_funnel','unordered_funnel_ablation'))

def validate_result(out, prior_stats=None, expected_sessions=2048):
    r=json.loads((out/'result.json').read_text())
    if r['status']!='ROUND10_SCREEN_COMPLETED' or set(r['arms'])!=set(ARMS):
        raise ValueError('Complete Round10 result required')
...
def create_report(out=OUT, prior_stats=None, expected_sessions=2048):
    ...
    title='OTTO Round 10 · Historical same-product action progression'
    primary=cmp['ordered_funnel_minus_control_shared']
    ...
    (out/'round10_report.html').write_text(html)
    save_json(out/'report_receipt.json',{'status':'ROUND10_REPORT_READY','result_sha256':sha(out/'result.json'),
        'plot_hashes':plot_hashes,'charts':9,'new_model_fits':0,'metrics_replayed':True})
    (out/'MILESTONE_REVIEW.md').write_text('# OTTO Round 10\n\n'+
        ...
        'Twelve challenger models and six existing control replays; no raw JSON scan, old-graph rebuild, control refit, GitHub/cloud writes or submission. Shared data extraction is accounted for separately. '+
        'Feature engineering remains open. Full source and experiment receipts are in this package.\n')
    print('RESULT: ROUND10_REPORT_READY',flush=True)
    return r

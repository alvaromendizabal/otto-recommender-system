...
class SyntheticScreen(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with mock.patch.object(e,'OUT',cls.out),mock.patch.dict('sys.modules',{'otto_recsys.research.dataset':module}),mock.patch.object(e,'control_prediction',side_effect=counted):
            ...
            if e.ROUND==10:
                # Genuine same-product funnels, including reversed controls.
                for sid in range(1,301):
                    item=(sid*7)%400
                    sequence=(0,1,2) if sid%2 else (2,1,0)
                    for offset,kind in enumerate(sequence,5):
                        histrows.append((sid,item,e.HISTORY_END-10000+offset*1000,kind,offset))
                histrows.sort(key=lambda r:(r[0],r[4]))
    ...
    def test_round10_fixture_has_nonzero_order_sensitive_features(self):
        if e.ROUND!=10:return

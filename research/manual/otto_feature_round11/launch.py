...
CAPS={'tests':120,'prepare':240,'representations':300,'features':260,'screen':260,'report':90}
...
def package_check():
    for line in (ROOT/'MANIFEST.sha256').read_text().splitlines():
        ...
        if rel.endswith('.ipynb'):
            signature=json.loads((ROOT/'notebook_sources.json').read_text())[rel]
            cells=json.loads(p.read_text())['cells']
            actual=[{'cell_type':c['cell_type'],'source':''.join(c['source']) if isinstance(c['source'],list) else c['source']} for c in cells]
            if actual!=signature:raise ValueError('Notebook source changed: '+rel)
        elif hashlib.sha256(p.read_bytes()).hexdigest()!=expected:raise ValueError('Package file changed: '+rel)
...
def collect():
    ...
    for p in sorted(out.rglob('*')):
        ...
        if rel.parts[0] in ('features','models','index','shared','representation_inputs','representations') and p.suffix!='.json':continue
    ...
    for pattern in ('validation/*.py','*.py','*.json','*.md','LOCAL_TEST_LOG.txt','MANIFEST.sha256','evidence/*','review/*.json','tests/*.py'):
        for candidate in sorted(ROOT.glob(pattern)):
            if candidate.is_file() and not candidate.is_symlink():files.append(candidate)
    files=sorted(set(files))
    size=sum(p.stat().st_size for p in files)
    if size>30*1024**2:raise ValueError('Unexpected large return bundle; local evidence preserved')
    manifest=[];target=ROOT/'otto_round11_return.zip';tmp=target.with_suffix('.assembling')
...
def _run_stage(phase):
    ...
    if phase not in ('report','bundle'):
        failed=[]
        prior_runs=list((out/'runs').glob('launcher-*.json'))
        for number in (11,12):
            other=Path.home()/f'otto_feature_round{number:02d}/outputs/runs'
            prior_runs+=list(other.glob('launcher-*.json'))
        for p in prior_runs:
            row=json.loads(p.read_text())
            if row.get('exit_code')!=0 or row.get('reason'):failed.append(p.name)
        if failed:raise RuntimeError('PRIOR_STOP_DETECTED: return the existing ZIP; do not repeat unchanged execution.')
    if phase in ('prepare','representations','features'):
        good=[json.loads(p.read_text()) for p in (out/'runs').glob('launcher-tests-*.json')]
        if not any(x.get('exit_code')==0 and not x.get('reason') for x in good):
            raise RuntimeError('Run the tests stage successfully first.')
    if phase=='representations' and not (out/'representation_inputs/manifest.json').is_file():
        raise RuntimeError('Complete preparation first.')
    if phase=='features' and not (out/'representations/manifest.json').is_file():
        raise RuntimeError('Complete representation fitting first.')
    if phase=='screen' and not (out/'feature_manifest.json').is_file():
        raise RuntimeError('Complete the feature stage successfully first.')
...
def run_stage(phase):
    import fcntl
    out=ROOT/'outputs';out.mkdir(exist_ok=True)
    with (Path.home()/'.otto_rounds08_09.lock').open('a') as lock:
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: python launch.py tests|prepare|representations|features|screen|report|bundle')

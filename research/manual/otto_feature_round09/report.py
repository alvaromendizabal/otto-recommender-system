...
LABELS={'control_shared':'134-feature saved control',
        'forward_continuation':'+ 24 forward-continuation features',
        'unsigned_window_ablation':'+ 24 unsigned-window features'}
PAIRS=(('forward_continuation','control_shared'),('unsigned_window_ablation','control_shared'),
       ('forward_continuation','unsigned_window_ablation'))

def validate_result(out, prior_stats=None, expected_sessions=2048):
    r=json.loads((out/'result.json').read_text())
    if r['status']!='ROUND09_SCREEN_COMPLETED' or set(r['arms'])!=set(ARMS):
        raise ValueError('Complete Round09 result required')
...
def create_report(out=OUT, prior_stats=None, expected_sessions=2048):
    ...
    title='OTTO Round 09 · Directed historical-session continuation'
    primary=cmp['forward_continuation_minus_control_shared']
    ...
    (out/'round09_report.html').write_text(html)
    save_json(out/'report_receipt.json',{'status':'ROUND09_REPORT_READY','result_sha256':sha(out/'result.json'),
        'plot_hashes':plot_hashes,'charts':9,'new_model_fits':0,'metrics_replayed':True})
    (out/'MILESTONE_REVIEW.md').write_text('# OTTO Round 09\n\n'+
    ...
    print('RESULT: ROUND09_REPORT_READY',flush=True)
    return r

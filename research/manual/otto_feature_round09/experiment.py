...
from shared_data import context as prepare_context,prior_chunks,require,evidence
from source_contract import HISTORY_END,FIT_END,COMMIT
from feature_logic import names,build
...
ARMS=('control_shared', 'forward_continuation', 'unsigned_window_ablation')
...
def feature_identity(ctx):
 ...
 return {'round':ROUND,'source_commit':COMMIT,'protocol_sha256':sha(ROOT/'protocol.json'),
   'code_sha256':{n:sha(ROOT/n) for n in ('experiment.py','shared_data.py','feature_logic.py','core.py')},
   'shared_input_contract_sha256':sha(d/'input_contract.json'),'shared_cohort_sha256':sha(d/'cohort.npz'),
   'shared_history_sha256':load_json(d/'history.json')['sha256'],
   'shared_postings_sha256':load_json(d/'postings.json')['sha256'],
...
def verify_new_chunk(path,ids,aids):
 with np.load(path,allow_pickle=False) as z:
  if set(z.files)!={'ids','aids','primary','ablation','neighbor_counts','multi_counts','overlap'}:raise ValueError('New chunk schema differs')
  ...
  if z['neighbor_counts'].shape!=(len(ids),2) or z['multi_counts'].shape!=(len(ids),2):raise ValueError('Neighbor diagnostic shape')
 if any(x.shape!=(len(ids),400,24) or x.dtype!=np.float32 or not np.isfinite(x).all() or (x<0).any() for x in (xx,yy)):
  raise ValueError('Invalid 24-feature representation')
 return xx,yy

def features(ctx):
 contract=feature_identity(ctx);save_json(OUT/'feature_contract.json',contract);cid=sha(OUT/'feature_contract.json')
 a,h,lookup,freq=evidence(ctx);parts=[];new_count=reused=0
 # Read-only shared extraction; each round commits its OWN feature checkpoints.
 for old,ids,candidates,base in prior_chunks(ctx):
  ...
  if receipt.exists():
   ...
   aa=[];bb=[];counts=[];multi=[];overlap=[]
   for j,sid in enumerate(ids):
    anchors=a['anchors'][s+j];anchors=anchors[anchors>=0]
    source_ids=sorted(set().union(*(lookup.get(int(x),set()) for x in anchors)))
    x,y,diag=build(ROUND,anchors,candidates[j],source_ids,h,freq)
    if s+j<8:
     xx,yy,_=build(ROUND,anchors,candidates[j,::-1],list(reversed(source_ids)),h,freq)
     if not np.array_equal(xx[::-1],x) or not np.array_equal(yy[::-1],y):raise ValueError('Candidate/source permutation invariant failed')
    aa.append(x);bb.append(y);counts.append(diag['neighbor_counts']);multi.append(diag['multi_anchor_counts']);overlap.append(diag['neighbor_identity_overlap'])
   digest=save_arrays(path,ids=ids,aids=candidates,primary=np.stack(aa),ablation=np.stack(bb),
      neighbor_counts=np.asarray(counts,np.int64),multi_counts=np.asarray(multi,np.int64),overlap=np.asarray(overlap,np.int64))
  ...
  PROGRESS.update(stage='historical_feature_chunks',completed=e,total=len(ctx['ids']));log('feature_checkpoint',**PROGRESS)
...
def screen(ctx):
    ...
    if (OUT/'result.json').exists():
        ...
        return {'status':'ROUND09_SCREEN_COMPLETED','new_model_fits':0,'models_reused':12}
    ...
    for fi,fold,tr,va,d,groups,yy,xt,mt,st,xv,mv,sv in prepared:
        ...
        for arm in ARMS[1:]:
            xtrain=np.concatenate([xt,mt if arm=='forward_continuation' else st],axis=1)
            validation=np.concatenate([xv,mv if arm=='forward_continuation' else sv],axis=1)
            allnames=names+list(NAMES if arm=='forward_continuation' else GLOBAL_NAMES);hits=np.zeros((len(va),3),dtype=np.int64)
            for j,o in enumerate(OBJECTIVES):
                before_unit()
    ...
    for left,right in [('forward_continuation','control_shared'),('unsigned_window_ablation','control_shared'),
                       ('forward_continuation','unsigned_window_ablation')]:
    ...
    result={'status':'ROUND09_SCREEN_COMPLETED','source_commit':COMMIT,'sessions':len(ids),'validation_sessions':len(d),
            ...
            'primary_comparison':'forward_continuation_minus_control_shared','secondary_comparisons_are_not_advancement_gates':True,'model_inventory':inv,
            ...
            'limitations':['Reused Round02 fitting cohort; adaptive exploration, not independent validation.',
              'Two predeclared independent families are evaluated against the same immutable 134-feature control.',
              'All original study sessions excluded from certified retained historical tails; source not full raw sessions.',
              '64 recent matching sessions per anchor; not exhaustive nearest-neighbor retrieval.',
...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('phase',choices=['prepare','postings','histories','features','screen'])
    ...
    try:
        with (OUT/'.experiment.lock').open('a') as lock:
            ...
            if args.phase in ('prepare','postings','histories'):
                if ROUND!=8:raise ValueError('Round09 may not rebuild shared source data')
                import shared_data
                details=getattr(shared_data,args.phase)(ctx,before_unit)
            else:
                if ROUND==9 and args.phase=='screen':
                    previous=Path.home()/'otto_feature_round08/outputs/result.json'
                    if not previous.exists() or load_json(previous).get('status')!='ROUND08_SCREEN_COMPLETED':
                        raise ValueError('Finish Round08 screen before Round09; its gain may be negative.')

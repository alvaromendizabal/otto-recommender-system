...
LABELS={'control_shared':'134-feature saved control',
        'repeat_context':'+ 12 repeat-conditioned affinity features',
        'global_context_ablation':'+ 12 global-context affinity features'}
PAIRS=(('repeat_context','control_shared'),('global_context_ablation','control_shared'),
       ('repeat_context','global_context_ablation'))

def validate_result(out, prior_stats=None, expected_sessions=2048):
    r=json.loads((out/'result.json').read_text())
    if r['status']!='ROUND06_SCREEN_COMPLETED' or set(r['arms'])!=set(ARMS):
        raise ValueError('Complete Round06 result required')
...
def create_report(out=OUT, prior_stats=None, expected_sessions=2048):
    ...
    f=go.Figure(go.Scatter(x=vals,y=['Repeat context − control','Global context − control','Repeat context − global context'],mode='markers',
    ...
    f=go.Figure()
    for row in contexts:
        f.add_trace(go.Bar(name='Training fold '+str(row['fold']),x=[x['name'] for x in first],y=row['mean_abs_primary_minus_global']))
    f.update_layout(title='Does repeat conditioning actually change the representation?',barmode='group',yaxis_title='Mean absolute difference from global context',height=690,xaxis_tickangle=-70);figures.append(f)
    ...
    title='OTTO Round 06 · Repeat-versus-discovery affinity context'
    primary=cmp['repeat_context_minus_control_shared']
    ...
    (out/'round06_report.html').write_text(html)
    save_json(out/'report_receipt.json',{'status':'ROUND06_REPORT_READY','result_sha256':sha(out/'result.json'),
        'plot_hashes':plot_hashes,'charts':9,'new_model_fits':0,'metrics_replayed':True})
    (out/'MILESTONE_REVIEW.md').write_text('# OTTO Round 06\n\n'+
        f"Control {r['arms']['control_shared']['weighted_recall_at_20']:.6f}; primary gain {primary['gain']:+.6f}.\n\n"+
        'Decision: '+primary['decision']+'. Within-repeat-stratum affinity context is compared with equal-width global candidate context of the same graph signals. Cached affinities are reused unchanged. '+
        'Twelve challenger models and six existing control replays; no raw JSON scan, new index, or control rebuild, GitHub/cloud writes or submission. '+
        'Feature engineering remains open. Full source and experiment receipts are in this package.\n')
    print('RESULT: ROUND06_REPORT_READY',flush=True)
    return r

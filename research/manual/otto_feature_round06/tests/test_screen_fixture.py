...
from relative_features import NAMES, SEEN_SIGNAL
...
class SyntheticScreen(unittest.TestCase):
    ...
    def test_both_arms_use_146_features(self):
        self.assertEqual({r['features'] for r in self.result['fold_results'] if r['arm']!='control_shared'},{146})
    ...
    def test_context_before_sampling(self):
        fm=json.loads((self.out/'feature_manifest.json').read_text());self.assertEqual(fm['rows'],len(self.ids)*400)

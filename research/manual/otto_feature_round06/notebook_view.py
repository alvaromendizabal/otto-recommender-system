...
def start():
    ...
    if (receipt['status']!='ROUND06_REPORT_READY' or hashlib.sha256(raw).hexdigest()!=receipt['result_sha256']):
        raise ValueError('Run the report phase successfully first')
    if result['status']!='ROUND06_SCREEN_COMPLETED':raise ValueError('Research comparison not complete')
    ...
    display(Markdown('**Decision:** '+result['comparisons']['repeat_context_minus_control_shared']['decision']+'\n\n'
                    '**Repeat-conditioned context primary gain versus matched control:** '+f"{result['comparisons']['repeat_context_minus_control_shared']['gain']:+.6f}"+
...
def show_chart(number):
    ...
    if number not in range(1,10):raise ValueError('Chart number must be 1..9')
    ...
    display({'application/vnd.plotly.v1+json':spec,'text/plain':f'Round 06 chart {number} (interactive Plotly)'},raw=True)
...
def finish():
    if STATE['chart_payloads_emitted']!=list(range(1,10)):raise ValueError('Run all nine saved-chart cells')
    persist('ROUND06_NOTEBOOK_REVIEW_COMPLETE')
    import subprocess
    print('ROUND06_NOTEBOOK_REVIEW_COMPLETE',flush=True)

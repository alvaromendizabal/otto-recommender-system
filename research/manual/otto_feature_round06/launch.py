...
CAPS={'tests':60,'features':200,'screen':260,'report':60}
...
def collect():
    ...
    for p in sorted(out.rglob('*')):
        ...
        if rel.parts[0] in ('features','models','index') and p.suffix!='.json':continue
    ...
    for pattern in ('*.py','*.json','*.md','LOCAL_TEST_LOG.txt','MANIFEST.sha256','evidence/*','review/*.json','tests/*.py'):
    ...
    manifest=[];target=ROOT/'otto_round06_return.zip';tmp=target.with_suffix('.assembling')
...
def _run_stage(phase):
    ...
    if phase in ('tests','features','screen'):
        failed=[]

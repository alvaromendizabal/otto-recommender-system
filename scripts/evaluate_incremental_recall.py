...
from otto_recsys.retrieval.incremental_recall import evaluate_incremental_recall
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Measure exact Item2Vec marginal target recovery beyond the "
            "co-visitation candidate family."
        )
    )
    ...
    parser.add_argument(
        "--temp-root",
        type=Path,
        default=Path("data/interim/duckdb_incremental_recall"),
    )
    ...
    logger = configure_logging("incremental_recall")

    result = evaluate_incremental_recall(
    ...
    print("OTTO_INCREMENTAL_RECALL_PASSED", flush=True)
    return 0
